...
test("trip planning creates a trip, adds scenes, reorders, and removes", async ({
  ...
  await page.getByRole("button", { name: "將 SLC-001 加入此日" }).click();
  ...
  await page.getByRole("button", { name: "上移 SLC-001" }).click();
  await expect(
    firstDayOrder.nth(0).getByRole("link", { name: "SLC-001" }),
  ).toBeVisible();
  ...
  await expect(page.getByText("1 個場景")).toBeVisible();
});
